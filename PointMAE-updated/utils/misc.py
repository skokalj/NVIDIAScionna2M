...
def fps(data, number):
    '''
        data B N C (C can be 3 for xyz or 6 for xyz+normals)
        number int
    '''
    data = data.contiguous()
    # FPS uses only xyz (first 3 channels) for distance computation
    xyz = data[:, :, :3].contiguous()
    fps_idx = pointnet2_utils.furthest_point_sample(xyz, number)
    # Gather all channels using the FPS indices
    fps_data = pointnet2_utils.gather_operation(data.transpose(1, 2).contiguous(), fps_idx).transpose(1,2).contiguous()
    return fps_data
