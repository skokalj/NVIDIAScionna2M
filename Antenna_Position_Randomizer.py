...
### this file moves a tx and rx object around a room and generated valid positions within the room to place the antennas
...
def check_collision_2d(bbox1, bbox2, margin=0.5):
...
bpy.ops.export_scene.mitsuba(filepath="C:/Users/sbjoh/Downloads/roomsMO/nosphere{0}.xml".format(1))
import bpy
import random
import math
import mitsuba as mi
...
path = ""
#this will save a number of camera placements and an xml
bpy.ops.export_scene.mitsuba(filepath="{1}/withsphere{0}.xml".format(2,path))
for i in range(5000):
...
np.save('{0}/txloc2.npy'.format(path), np.array(txlocs, dtype=object), allow_pickle=True)
np.save('{0}/rxloc2.npy'.format(path), np.array(rxlocs, dtype=object), allow_pickle=True)
