...
SWEEP_CONFIGS = [
    {
        ...
        "xml_extra_blocks": 0,
        "zr_extra_blocks": 0,
        "description": "Original settings, train longer (800 epochs)",
    },
    {
        "run": "run_E",
        "lr": 1e-4,
        ...
        "xml_extra_blocks": 0,
        "zr_extra_blocks": 0,
        "description": "Larger batch (512), train longer (800 epochs)",
    },
    {
        "run": "run_F",
        "lr": 1e-4,
        ...
        "batch_size": 512,
        "num_epochs": 800,
        "xml_extra_blocks": 2,   # adds 2 extra ResBlock1D to ZXMLEncoder
        "zr_extra_blocks": 1,    # adds 1 extra ResBlock1D to ZREncoder
        "description": "Larger batch (512) + bigger encoders + 800 epochs",
    },
]
...
class ZXMLEncoder(nn.Module):
    def __init__(self, out_dim=128, extra_blocks=0):
        super().__init__()
        ...
        self.blocks = nn.Sequential(
            ResBlock1D(128),
            ResBlock1D(128),
            *[ResBlock1D(128) for _ in range(extra_blocks)],
        )
...
class SceneClipModelPad(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.zr_encoder = ZREncoder(zr_model_emb_dim, extra_blocks=zr_extra_blocks)
        self.xml_encoder = ZXMLEncoder(xml_model_emb_dim, extra_blocks=xml_extra_blocks)
...
for cfg in SWEEP_CONFIGS:
    print(f"\n{'='*60}")
    print(f"  Starting {cfg['run']}: {cfg['description']}")
    print(f"  lr={cfg['lr']}  weight_decay={cfg['weight_decay']}  batch_size={cfg['batch_size']}  epochs={cfg['num_epochs']}")
    print(f"  xml_extra_blocks={cfg['xml_extra_blocks']}  zr_extra_blocks={cfg['zr_extra_blocks']}")
    print(f"{'='*60}\n")

    train_dataset = PairedContrastiveDataset(train_A, train_B, transform_A=transform_AT, transform_B=transform_BT)
    val_dataset   = PairedContrastiveDataset(val_A, val_B, transform_A=transform_AV, transform_B=transform_BV)
    ...
    trainer.fit(SceneClipModelPad(
                ...
                xml_extra_blocks=cfg["xml_extra_blocks"],
                zr_extra_blocks=cfg["zr_extra_blocks"],
            ), train_dataloaders=trainloader,
                        val_dataloaders=valloader)

    print(f"\n  {cfg['run']} done. Best val_loss: {checkpoint_cb.best_model_score:.4f}")
...
print("  SWEEP SUMMARY")
print("="*60)
df = pd.DataFrame(results)[["run", "description", "batch_size", "num_epochs", "xml_extra_blocks", "best_val_loss"]]
print(df.to_string(index=False))
df.to_csv("sweep_results2.csv", index=False)
print("\nResults saved to sweep_results2.csv")